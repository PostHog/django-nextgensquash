...
from nextgensquash import planning
...
class CycleBreaker:
    ...
    def run_before_break_edges(self, squasher: planning.Squasher) -> list[tuple[str, str, str, str]]:
        """`run_before` entries on old migrations that point at an old migration of
        an app applied *earlier* in our order. The target inherits the source as
        a parent through the replaces redirect, so the entry is a back edge in
        the same sense as `cycle_break_edges`; it is removed from the source
        file's `run_before` list. Entries whose target stays live are carried
        onto the squash by the emitter instead.

        Returns (source_app, source_name, target_app, target_name).
        """
        pos = {a: i for i, a in enumerate(self.apply_order)}
        out: list[tuple[str, str, str, str]] = []
        for m in squasher.old.values():
            for rb in m.run_before:
                ...
                if rb.app not in pos or m.ref.app not in pos:
                    continue
                if pos[m.ref.app] > pos[rb.app]:
                    out.append((m.ref.app, m.ref.name, rb.app, rb.name))
        return sorted(set(out))
