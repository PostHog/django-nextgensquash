...
class Emitter:
        ...
        for ms in self._models_in_app():
            ...
            for fname, field in ms.fields.items():
                ...
                if t_app in latest_old:
                    ...
                    # BUILTIN_FK_APPS or Config.ignored_apps deliberately.
                    raise RuntimeError(
    ...
    def build(self) -> list[SquashFile]:
        ...
        early_names = self.config.early_models.get(self.app, frozenset())
        ...
        for mname in sorted(early_names):
            ...
            assert not self._intra_app_fk_targets(ms, set()), (
                f"{self.app}.{mname} is in Config.early_models but references other models; "
        ...
        files: list[SquashFile] = [stub, initial] if emit_stub else [initial]
    ...
    def _single_leaf(self, files: list[SquashFile]) -> list[SquashFile]:
        ...
        Never applied when the initial is the leaf: the initial is a replacement
        node, young dependencies remap onto it, and an initial -> young edge
        would invert into a cycle.
        """
        if files[-1].name == self.INITIAL_NAME:
            return files
