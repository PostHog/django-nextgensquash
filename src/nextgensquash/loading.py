...
from nextgensquash.config import Config, managed_app_migration_dirs
...
class MigrationTree:
    ...
    def _discover_files(config: Config) -> list[Path]:
        files: list[Path] = []
        for root in managed_app_migration_dirs(config).values():
            if not root.is_dir():
                continue
    ...
    @classmethod
    def load(cls, repo_root: Path, config: Config) -> MigrationTree:
        files = cls._discover_files(config)
        dates = GitDates(repo_root).first_added(files)
        loader = MigrationLoader(connection=None, ignore_no_migrations=True)
        managed = set(managed_app_migration_dirs(config))
    ...
    def partition(
        ...
        for k, m in self.migrations.items():
            if m.ref.app in self.config.frozen_apps:
                young[k] = m
                continue
