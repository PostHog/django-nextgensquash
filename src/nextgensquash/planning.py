    ...
    def _rebalance_min_young(self, min_young: int) -> None:
        ...
        for app, migs in loading.MigrationTree.group_by_app(self.old).items():
            ...
            for m in reversed(movable):
                ...
                if m.replaces or m.ref.key in blocked or is_state_move or self._runs_before_old(m):
                    break
            ...
            for y in self.young.values():
                for op in y.operations:
                    ...
                    if op.kind in self._SQL_REFERENCING_OP_KINDS and op.target:
                        names.add(op.target)
                    if op.sql:
                        # A young op that creates the name itself is order-
                        # independent: forwarded creates are rewritten to
                        # IF NOT EXISTS, so only a use outside the CREATE
                        # head (DROP, ALTER, VALIDATE) needs the object first.
                        sql_without_creates = op.sql
                        for rx in self._SQL_CREATED_NAME_RES:
                            sql_without_creates = rx.sub("", sql_without_creates)
                        # Whole-token match: a bare identifier must not match
                        # inside a longer word or an unrelated name.
                        names.update(
                            n for n in created if re.search(rf"\b{re.escape(n)}\b", sql_without_creates) is not None
                        )
